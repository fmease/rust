...
use rustc_hir_analysis::astconv::HirLowerer;
    ...
    fn finish_resolving_struct_path(
        ...
        match *qpath {
            QPath::Resolved(ref maybe_qself, path) => {
                ...
                let ty = self.lowerer().lower_res_to_ty(self_ty, path, hir_id, true);
            ...
            QPath::TypeRelative(qself, segment) => {
                ...
                let result = self
                    .lowerer()
                    .lower_assoc_path_to_ty(hir_id, path_span, ty.raw, qself, segment, true);
                let ty = result
                    .map(|(ty, _, _)| ty)
