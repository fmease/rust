...
fn associated_type_bounds<'tcx>(
    ...
    let mut bounds = icx.lowerer().compute_bounds(item_ty, hir_bounds, PredicateFilter::All);
    // Associated types are implicitly sized unless a `?Sized` bound is found
    icx.lowerer().add_implicitly_sized(&mut bounds, item_ty, hir_bounds, None, span);
