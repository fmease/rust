...
use rustc_infer::traits::{ImplSource, ImplSourceUserDefinedData, SelectionError};
use rustc_middle::ty::elaborate::elaborate;
use rustc_middle::ty::{self, Ty, TyCtxt, TypeVisitableExt, Upcast};
use rustc_span::def_id::DefId;
use rustc_trait_selection::errors::UnableToConstructConstantValue;
use rustc_trait_selection::infer::TyCtxtInferExt;
use rustc_trait_selection::infer::region_constraints::{ConstraintKind, RegionConstraintData};
use rustc_trait_selection::regions::OutlivesEnvironmentBuildExt;
use rustc_trait_selection::traits::project::ProjectAndUnifyResult;
use rustc_trait_selection::traits::{
    EvaluateConstErr, Obligation, ObligationCause, ObligationCtxt, PredicateObligation,
    SelectionContext, project, try_evaluate_const,
};
use tracing::debug;

use super::{ImplInfo, ImplKind, Log, RegionDeps, RegionTarget};
...
/// in the future.
pub(super) fn find_auto_trait_generics<'tcx>(
    tcx: TyCtxt<'tcx>,
    self_ty: Ty<'tcx>,
    trait_def_id: DefId,
    ...
    if _log.extra {
        eprintln!("[[lgcy]]      orig_env:  {:?}", orig_env.caller_bounds());
    }

    let Some((new_env, user_env)) =
        evaluate_predicates(&infcx, trait_def_id, self_ty, orig_env, orig_env, &mut fresh_preds)
    else {
        return ImplKind::Negative;
    };

    if _log.extra {
        eprintln!("[[lgcy]]       new_env:  {:?}", new_env.caller_bounds());
        eprintln!("[[lgcy]]      user_env:  {:?}", user_env.caller_bounds());
    }

    // [[HIT]]  tests/rustdoc/synthetic_auto/complex.rs (would otherwise drop projection for Inner to Outer)
    // [[HIT]]  tests/rustdoc/synthetic_auto/project.rs (would otherwise drop projection from Inner to Outher)
    // [[HIT]]  tests/rustdoc-ui/synthetic-auto-trait-impls/lifetime-generic-user-impl-normalize.rs
    let (full_env, full_user_env) =
        evaluate_predicates(&infcx, trait_def_id, self_ty, new_env, user_env, &mut fresh_preds)
            .unwrap_or_else(|| {
                panic!("Failed to fully process: {self_ty:?} {trait_def_id:?} {orig_env:?}")
            });
    // let (full_env, full_user_env) = (new_env, user_env);

    if _log.extra {
        eprintln!("[[lgcy]]       full_env:  {:?}", full_env.caller_bounds());
        eprintln!("[[lgcy]]  full_user_env:  {:?}", full_user_env.caller_bounds());
    }

    debug!("find_auto_trait_generics({:?}): fulfilling with {:?}", trait_ref, full_env);

    // At this point, we already have all of the bounds we need. FulfillmentContext is used
    // to store all of the necessary region/lifetime bounds in the InferContext, as well as
    // an additional sanity check.
    let ocx = ObligationCtxt::new(&infcx);
    ocx.register_bound(ObligationCause::dummy(), full_env, self_ty, trait_def_id);
    let errors = ocx.select_all_or_error();
    if !errors.is_empty() {
        panic!("Unable to fulfill trait {trait_def_id:?} for '{self_ty:?}': {errors:?}");
    }
...
fn evaluate_predicates<'tcx>(
    ...
    for predicate in param_env.caller_bounds() {
        fresh_preds.insert(infcx.freshen(predicate.as_predicate()));
    }
    ...
    while let Some(pred) = predicates.pop_front() {
        ...
        match result {
            Ok(Some(ref impl_source)) => {
                ...
                if let ImplSource::UserDefined(ImplSourceUserDefinedData { impl_def_id, .. }) =
                    impl_source
                {
            ...
            Err(SelectionError::Unimplemented) => {
                if is_param_no_infer(pred.skip_binder().trait_ref.args) {
                    already_visited.remove(&pred);
                    add_user_pred(&mut user_computed_preds, pred.upcast(tcx));
    ...
    user_computed_preds.retain(|&old_pred| {
        ...
        ) = (new_pred.kind().skip_binder(), old_pred.kind().skip_binder())
        {
            if new_trait.def_id() == old_trait.def_id() {
                ...
                // NOTE(fmease): This doesn't consider const params which seems bad.
...
pub(super) fn map_vid_to_region<'cx>(
...
fn is_param_no_infer<'tcx>(args: ty::GenericArgsRef<'tcx>) -> bool {
    is_of_param(args.type_at(0)) && !args.types().any(|t| t.has_infer_types())
}

fn is_of_param<'tcx>(ty: Ty<'tcx>) -> bool {
    match ty.kind() {
        ty::Param(_) => true,
        ty::Alias(ty::Projection, p) => is_of_param(p.self_ty()),
        _ => false,
    }
}

fn is_self_referential_projection<'tcx>(
    tcx: TyCtxt<'tcx>,
    p: ty::PolyProjectionPredicate<'tcx>,
) -> bool {
    if let Some(ty) = p.term().skip_binder().as_type() {
        matches!(ty.kind(), ty::Alias(ty::Projection, proj) if proj == &p.skip_binder().projection_term.expect_ty(tcx))
    } else {
        false
    }
}

fn evaluate_nested_obligations<'tcx>(
    ty: Ty<'_>,
    ...
    for obligation in nested {
        let is_new_pred = fresh_preds.insert(selcx.infcx.freshen(obligation.predicate));
        ...
        match bound_predicate.skip_binder() {
                ...
                ty::PredicateKind::Clause(ty::ClauseKind::Projection(p)) => {
                    ...
                    if is_param_no_infer(p.skip_binder().projection_term.args)
                        ...
                        && is_new_pred
                    {
                        ...
                        if is_self_referential_projection(tcx, p) {
                            debug!(
                                "evaluate_nested_obligations: encountered a projection
                                 predicate equating a type with itself! Skipping"
                            );
                        } else {
                            add_user_pred( computed_preds, predicate);
                        }
                    }
                    ...
                    match project::poly_project_and_unify_term(selcx, &obligation.with(tcx, p))
                    {
