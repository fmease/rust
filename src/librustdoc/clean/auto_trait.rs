...
use rustc_trait_selection::solve::inspect::{
    InferCtxtProofTreeExt, InspectGoal, ProbeKind, ProofTreeVisitor,
};
...
fn find_auto_trait_generics<'tcx>(
    ...
    for polarity in [ty::PredicatePolarity::Positive, ty::PredicatePolarity::Negative] {
        let pred = ty::TraitPredicate { trait_ref, polarity };
        let goal = Goal::new(tcx, param_env, pred);
...
struct HasUserWrittenImpl;

impl<'tcx> ProofTreeVisitor<'tcx> for HasUserWrittenImpl {
    ...
    fn visit_goal(&mut self, goal: &InspectGoal<'_, 'tcx>) -> Self::Result {
        for candidate in goal.candidates() {
            // Any user-written impl counts even if inapplicable.
            if let ProbeKind::TraitCandidate { source: CandidateSource::Impl(_), .. } =
                candidate.kind()
            {
...
// FIXME(fmease): How do deal with ambiguities?
// FIXME(fmease): We do want to collect `<?#t as …>` which are ambiguous
//                as they can be generated for the projection term.
// FIXME(fmease): Check if we correctly deal with negative impls in the proof tree.
// FIXME(fmease): Check if we correctly deal with higher-ranked goals & candidates.
// FIXME(fmease): Check if we correctly deal with `WellFormed` goals
// FIXME(fmease): Meticulously consider trait pred polarity (`negative_bounds`)
// FIXME(fmease): Check if we correctly deal with overflow / diverging aliases
// FIXME(fmease): Check if we correctly deal with opaque types
impl<'tcx> ProofTreeVisitor<'tcx> for PredicateCollector<'tcx> {
    ...
    fn visit_goal(&mut self, goal: &InspectGoal<'_, 'tcx>) -> Self::Result {
        ...
        for nested_goal in nested_goals {
            // FIXME(fmease): Make rustc's `visit_with` public & use it here.
            if nested_goal.depth() < self.config().max_depth {
                self.visit_goal(&nested_goal)?;
            }
        }
...
#[derive(Debug)]
enum ImplKind<'tcx> {
    Explicit, // FIXME: Bad name
    Positive(ImplInfo<'tcx>),
    Negative,
}
