...
pub(crate) enum ItemKind {
    ...
    TyAssocConstItem(Generics, Type),
    /// An associated constant in a trait impl or a provided one in a trait declaration.
    AssocConstItem(Generics, Type, ConstantKind),
...
// FIXME(fmease): #prePR investigate why this type needs `PartialEq`, `Eq`, `Hash`.
// If it actually doesn't (or we can easily get rid of it), remove the added derives
// from `Generics` and `WherePredicate` again.
#[derive(Clone, PartialEq, Eq, Hash, Debug)]
...
mod size_asserts {
    ...
    // FIXME(fmease): #prePR reduce the size again, it used to be 64
